...
class ProductionManagementAgent {
  ...
  formatScriptForTTS(script) {
    ...
    // Add main content. Section titles are for internal/on-screen structure
    // only — narrating "Section 1: ..." out loud is exactly the kind of
    // robotic tell that makes narration sound obviously AI-generated, so the
    // spoken track just flows from one section's content into the next.
  ...
  async generateCaptions(productionData) {
    ...
    // Prefer real transcription of the generated narration so captions are
    // synced to actual speech — estimated per-section durations can drift
    // significantly from how fast the TTS voice actually talks.
    const audioPath = productionData.assets.audio?.path;
    const isRealAudio = audioPath && !productionData.assets.audio.simulated;

    if (isRealAudio) {
      try {
        const segments = await this.aiVideoGenerator.transcribeAudioToSegments(audioPath);
        if (segments.length > 0) {
          await fs.writeFile(captionsPath, this.aiVideoGenerator.segmentsToSRT(segments));
        ...
        this.logger.warn(`Transcription-based captions failed, falling back to estimated timing: ${error.message}`);
      }
    }

    // Fallback: estimate timing from script section durations
  ...
  async assembleVideo(productionData) {
    ...
    try {
      ...
      // caption upload requires an OAuth scope this app doesn't currently have.
      const captionsPath = productionData.assets.captions?.path;
      if (process.env.BURN_IN_CAPTIONS === 'true' && captionsPath && captionsPath.endsWith('.srt')) {
        try {
          const captionedPath = finalVideoPath.replace('.mp4', '_captioned.mp4');
          await this.aiVideoGenerator.burnCaptions(finalVideoPath, captionsPath, captionedPath);
          await fs.unlink(finalVideoPath);
          await fs.rename(captionedPath, finalVideoPath);
        } catch (error) {
          this.logger.warn(`Caption burn-in failed, publishing without burned-in captions: ${error.message}`);
        }
      }
